...
def binary_search(l: list[T], item: T, key= lambda x:x) -> int:
    """
    ...
    """
    return _binary_search_aux(l, item, 0, len(l), key)

def _binary_search_aux(l: list[T], item: T, lo: int, hi: int, key = lambda x:x) -> int:
    """
    ...
    """
    if lo == hi:
        raise KeyError("the",item,"is not exist in here.")
    mid = (hi + lo) // 2
    if key(l[mid]) > key(item):
        # Item would be before mid
        return _binary_search_aux(l, item, lo, mid, key)
    elif key(l[mid]) < key(item):
        # Item would be after mid
        return _binary_search_aux(l, item, mid+1, hi,key)
    elif key(l[mid]) == key(item):
        return mid
