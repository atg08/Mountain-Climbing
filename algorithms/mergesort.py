...
def mergediff(l1: list[T], l2: list[T], key=lambda x:x) -> list[T]:
    new_list = []
    cur_left = 0
    cur_right = 0
    l1 = mergesortdiff(l1)
    l2 = mergesortdiff(l2)
    while cur_left < len(l1) and cur_right < len(l2):
        if key(l1[cur_left].difficulty_level) < key(l2[cur_right].difficulty_level):
            new_list.append(l1[cur_left])
            cur_left += 1
        elif key(l1[cur_left].difficulty_level) > key(l2[cur_right].difficulty_level):
            new_list.append(l2[cur_right])
            cur_right += 1
        else:
            if l1[cur_left].name < l2[cur_right].name:
...
def mergesortdiff(l: list[T]) -> list[T]:
    """
    Sort a list using the mergesort operation.
    :complexity: Best/Worst Case O(NlogN * comp(T))
    """
    if len(l) <= 1:
        return l
    break_index = (len(l)+1) // 2
    l1 = mergesortdiff(l[:break_index])
    l2 = mergesortdiff(l[break_index:])
    return mergediff(l1, l2)